document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadDataAndRender() {
        ...
        if (!rawData) { alert('Error: Could not find data.'); return; }
    ...
    function renderTable(data) {
        ...
        tableBody.innerHTML = '';
        if (data.length === 0) { tableBody.innerHTML = '<tr><td colspan="7" class="text-center">No blockers match the current filters.</td></tr>'; return; }
        data.forEach(item => {
            ...
            const row = `<tr><td>${item['Blocker Name'] || ''}</td><td>${item.Description || ''}</td><td>${item.Responsible || ''}</td><td>${item['Start Date'] || ''}</td><td><span class="badge text-capitalize bg-${priority === 'high' ? 'danger' : (priority === 'medium' ? 'warning text-dark' : 'primary')}">${priority}</span></td><td><span class="badge text-capitalize bg-${status === 'open' ? 'danger' : (status === 'in progress' ? 'warning text-dark' : 'success')}">${status}</span></td><td><button class="btn btn-sm btn-outline-primary edit-btn" data-index="${originalIndex}"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger delete-btn" data-index="${originalIndex}"><i class="bi bi-trash"></i></button></td></tr>`;
