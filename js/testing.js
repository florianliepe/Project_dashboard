document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderStatusPieChart(data) {
        ...
        if (!ctx) return;
        const statusCounts = data.reduce((acc, item) => { const status = String(item.Status || 'Not Started').toLowerCase(); acc[status] = (acc[status] || 0) + 1; return acc; }, {});
        const chartConfig = { type: 'doughnut', data: { labels: Object.keys(statusCounts).map(s => s.charAt(0).toUpperCase() + s.slice(1)), datasets: [{ data: Object.values(statusCounts), backgroundColor: ['#198754', '#0d6efd', '#6c757d', '#dc3545', '#ffc107'], borderColor: '#fff', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right' } } } };
    ...
    function renderTable(data) {
        ...
        tableBody.innerHTML = '';
        if (data.length === 0) { tableBody.innerHTML = '<tr><td colspan="8" class="text-center">No tests match the current filters.</td></tr>'; return; }
        ...
        data.forEach(item => {
            ...
            const row = `<tr class="${overdueClass}"><td><strong>${item['Activity Title'] || ''}</strong></td><td style="white-space: pre-wrap;">${item.Description || ''}</td><td>${dueDate ? dueDate.toLocaleDateString('de-DE') : (item['Due Date'] || '')}</td><td class="text-capitalize ${statusClass}">${item.Status || 'N/A'}</td><td><div class="progress" style="height: 22px; font-size: 0.9em;"><div class="progress-bar" role="progressbar" style="width: ${progress}%;" aria-valuenow="${progress}">${progress}%</div></div></td><td>${item.Responsible || ''}</td><td>${item.Blocker || 'None'}</td><td class="text-center"><button class="btn btn-sm btn-outline-primary edit-btn mb-1" data-index="${originalIndex}"><i class="bi bi-pencil"></i></button> <button class="btn btn-sm btn-outline-danger delete-btn" data-index="${originalIndex}"><i class="bi bi-trash"></i></button></td></tr>`;
